...
export default function SelectBiggestMechanic({ level, onComplete, onFail }) {
  const [options, setOptions] = useState([]);
  const [biggestId, setBiggestId] = useState(null);
  ...
  function generateRound() {
    ...
    const biggestSize = Math.max(...shuffledSizes);

    const newOptions = shuffledSizes.map((size, index) => ({
      id: `item-${index}`,
      size,
      color: colorsData[Math.floor(Math.random() * colorsData.length)].hex,
      isBiggest: size === biggestSize
    }));

    setOptions(newOptions);
    setBiggestId(newOptions.find(o => o.isBiggest).id);
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.instruction}>
        ...
        <Text style={styles.targetText}>BIGGEST</Text>
